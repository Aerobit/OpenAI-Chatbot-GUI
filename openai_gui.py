...
class ChatbotUI:
    def __init__(self):
        ...
        self.custom_model_label = tk.StringVar(value="Custom...")
        self.custom_temperature_label = tk.StringVar(value="Custom...")
        self.custom_max_tokens_label = tk.StringVar(value="Custom...")
    ...
    def setup_ui(self):
        ...
        helpmenu.add_command(label="Change API Key", command=self.api_key_entry_popup)
        ...
        self.conversation.bind("<Button-3>", self.show_context_menu)
        self.context_entry.bind("<Button-3>", self.show_context_menu)
        self.user_input.bind("<Button-3>", self.show_context_menu)

    def show_context_menu(self, event):
        self.context_menu.post(event.x_root, event.y_root)
        self.context_menu.event_source_widget = event.widget

    def select_all_text(self):
        widget = self.context_menu.event_source_widget
    ...
    def setup_settings_menu(self):
        ...
        self.settings_menu.add_cascade(label="Model", menu=self.create_submenu(self.settings_menu, self.model_var, models, self.custom_model_label, gpt_3_5_turbo_menu))
        ...
        self.settings_menu.add_cascade(label="Temperature", menu=self.create_submenu(self.settings_menu, self.temperature_var, temperatures, self.custom_temperature_label))
        ...
        self.settings_menu.add_cascade(label="Max Tokens", menu=self.create_submenu(self.settings_menu, self.max_tokens_var, max_tokens, self.custom_max_tokens_label))

    def create_submenu(self, parent_menu, variable, options, custom_label_var, gpt_3_5_turbo_menu=None):
        ...
        created_menu.add_separator()
        created_menu.custom_index = tk.END
        created_menu.add_radiobutton(label=custom_label_var.get(), variable=variable, value="custom", command=lambda: self.custom_input(variable, created_menu, custom_label_var))
        variable.trace_add("write", lambda *args: self.reset_custom_label(variable, created_menu, custom_label_var))
        return created_menu

    def reset_custom_label(self, variable, menu, custom_label_var):
        current_value = variable.get()
        if current_value != "custom":
            menu.delete(menu.custom_index)
            custom_label_var.set("Custom...")
            menu.add_radiobutton(label=custom_label_var.get(), variable=variable, value="custom", command=lambda: self.custom_input(variable, menu, custom_label_var))
            menu.custom_index = tk.END

    def on_menu_open(self, event, variable, menu, custom_label_var):
        if menu.index(tk.ACTIVE) is not None:
            self.reset_custom_label(variable, menu, custom_label_var)

    def custom_input(self, variable, menu, custom_label_var):
        value = simpledialog.askstring("Custom Input", f"Enter custom value for {variable._name}:")
        if value:
            variable.set(value)
            menu.delete(menu.custom_index)
            custom_label_var.set(f"Custom ({value})")
            menu.add_radiobutton(label=custom_label_var.get(), variable=variable, value=value, command=lambda: self.custom_input(variable, menu, custom_label_var))  # add new custom option with updated label
            menu.custom_index = tk.END

            if variable is self.model_var:
                self.custom_model_label.set("Custom...")
            elif variable is self.temperature_var:
